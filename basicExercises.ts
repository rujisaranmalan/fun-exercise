function personalInfo(namesAndAges: [string , number]) {
...
function onlyEvens(arr: number[]): number[] {
...
function findLongestName(names: string[]): string | undefined {
...
function cutCommonSubstringWithPlaceholder(words: string[]){
    if(words.length === 0){
        return []
    }
...
function mostCommonField<T>(arr: T[], field: keyof T): T[keyof T] | undefined {
  ...
  let maxCount = 0;
  let winner: T[keyof T] | undefined = undefined;
